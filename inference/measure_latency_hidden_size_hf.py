...
os.makedirs(f"{gpu_type}/results_hidden_size", exist_ok=True)
    ...
    for variant in parameter_dict.keys():
        ...
        while True:
            ...
            if total_tokens_generated_per_prompt == output_tokens:
                trial += 1
                with open(
                    f"{gpu_type}/results_hidden_size/hs_hf_latency_hs_{hidden_size}_layers_{num_hidden_layers}_bs_{num_prompts}_InputTokens_{input_tokens}_OutputTokens_{output_tokens}_trial_{trial}.txt",
                    'w'
                ) as f:
