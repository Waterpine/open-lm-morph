...
with open(f"config_files/{gpu_type}_layers.json", 'r') as file:
    json_str = file.read()
    parameter_dict = json.loads(json_str)

os.makedirs(f"{gpu_type}_rebuttal/results_layers", exist_ok=True)
...
for num_prompts in num_prompts_list:
    for variant in parameter_dict.keys():
        ...
        checkpoint_path = "my_llama_checkpoint_layers_hf"
        ...
        while True:
            ...
            if total_tokens_generated_per_prompt == output_tokens:
                trial += 1
                with open(
                    f"{gpu_type}_rebuttal/results_layers/layers_hf_latency_hs_{hidden_size}_layers_{num_hidden_layers}_bs_{num_prompts}_InputTokens_{input_tokens}_OutputTokens_{output_tokens}_trial_{trial}.txt",
                    'w'
                ) as f:
                    f.write(f"Inference latency: {latency:.4f} seconds")
                print(f"layers_hf_hs_{hidden_size}_layers_{num_hidden_layers}_bs_{num_prompts}_InputTokens_{input_tokens}_OutputTokens_{output_tokens}_trial_{trial}")
