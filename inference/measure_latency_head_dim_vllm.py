...
with open(f"config_files/{gpu_type}_head_dim.json", 'r') as file:
    json_str = file.read()
    parameter_dict = json.loads(json_str)

os.makedirs(f"{gpu_type}/results_head_dim_vllm", exist_ok=True)
...
for num_prompts in num_prompts_list:
    for variant in parameter_dict.keys():
        ...
        n_heads = parameter_dict[variant]["n_heads"]
        ...
        assert head_dim == config.hidden_size // config.num_attention_heads
        config.torch_dtype = torch.bfloat16

        model = LlamaForCausalLM(
            config=config
        )
        checkpoint_path = f"my_llama_checkpoint_7b_{input_tokens}"
        ...
        while True:
            ...
            if total_tokens_generated_per_prompt == output_tokens:
                trial += 1
                with open(
                    f"{gpu_type}/results_head_dim_vllm/hs_vllm_latency_hs_{hidden_size}_nh_{n_heads}_layers_{num_hidden_layers}_bs_{num_prompts}_InputTokens_{input_tokens}_OutputTokens_{output_tokens}_trial_{trial}.txt",
                    'w'
                ) as f:
                    f.write(f"Inference latency: {latency:.4f} seconds")
                print(
                    f"hs_vllm_hs_{hidden_size}_nh_{n_heads}_layers_{num_hidden_layers}_bs_{num_prompts}_InputTokens_{input_tokens}_OutputTokens_{output_tokens}_trial_{trial}")
