...
os.makedirs(f"{gpu_type}/results_hidden_size_vllm", exist_ok=True)
...
for num_prompts in num_prompts_list:
    for variant in parameter_dict.keys():
        ...
        checkpoint_path = f"my_llama_checkpoint_hidden_size_vllm"
        ...
        while True:
            ...
            if total_tokens_generated_per_prompt == output_tokens:
                trial += 1
                with open(
                    f"{gpu_type}/results_hidden_size_vllm/hs_vllm_latency_hs_{hidden_size}_layers_{num_hidden_layers}_bs_{num_prompts}_InputTokens_{input_tokens}_OutputTokens_{output_tokens}_trial_{trial}.txt",
                    'w'
                ) as f:
                    f.write(f"Inference latency: {latency:.4f} seconds")
                print(
                    f"hs_vllm_hs_{hidden_size}_layers_{num_hidden_layers}_bs_{num_prompts}_InputTokens_{input_tokens}_OutputTokens_{output_tokens}_trial_{trial}")
