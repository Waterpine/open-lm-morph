...
def func(X, A, B, E, alpha, gamma):
    N, D, height, dim = X
    return (E + A / np.power(N, alpha) + B / np.power(D, alpha)) * (1 + gamma * np.power(dim / height, alpha))
...
for model_size in model_size_fit_20N_list:
    model_variants = random.choice(model_variants_fit_20N_list[model_size])
...
for model_size in model_size_fit_160N_list:
    ...
    model_variants = random.choice(model_variants_fit_160N_list[model_size])
    print(model_variants)
    N_data_fit_list.append(parameters_fit_dict[model_variants])
...
print(dim_data_fit)
print(loss_data_fit)
popt, pcov = curve_fit(func, (N_data_fit, D_data_fit, height_data_fit, dim_data_fit), loss_data_fit, maxfev=100000)
print(popt)
A = popt[0]
B = popt[1]
E = popt[2]
...
for idx in range(len(N_data_fit)):
    ...
    loss_predict = (E + A / np.power(N_data_fit[idx], alpha) + B / np.power(D_data_fit[idx], beta)) * (1 + gamma * np.power(dim_data_fit[idx] / height_data_fit[idx], alpha))
...
dim_v6_1b = 2816
dim_v8_1b = 3072
loss_v1_1b = (E + A / np.power(N_v1_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v1_1b / height_v1_1b, alpha))
print(f"loss_v1_1b: {loss_v1_1b}, error: {abs(loss_v1_1b - 2.8960) / 2.8960}")
loss_v3_1b = (E + A / np.power(N_v3_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v3_1b / height_v3_1b, alpha))
print(f"loss_v3_1b: {loss_v3_1b}, error: {abs(loss_v3_1b - 2.9090) / 2.9090}")
loss_v6_1b = (E + A / np.power(N_v6_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v6_1b / height_v6_1b, alpha))
print(f"loss_v6_1b: {loss_v6_1b}, error: {abs(loss_v6_1b - 2.9326) / 2.9326}")
loss_v8_1b = (E + A / np.power(N_v8_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v8_1b / height_v8_1b, alpha))
...
for idx in range(len(N_data_test)):
    ...
    loss_predict = (E + A / np.power(N_data_test[idx], alpha) + B / np.power(D_data_test[idx], beta)) * (1 + gamma * np.power(dim_data_test[idx] / height_data_test[idx], alpha))
...
def func_chinchilla(X, A, B, E, alpha):
    N, D = X
...
print(D_data_fit)
print(loss_data_fit)
popt, pcov = curve_fit(func_chinchilla, (N_data_fit, D_data_fit), loss_data_fit, maxfev=100000)
print(popt)
A = popt[0]
B = popt[1]
E = popt[2]
