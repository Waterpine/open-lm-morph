...
def func(X, A, B, E, alpha, gamma, eta):
    N, D, height, dim = X
    return (E + A / np.power(N, alpha) + B / np.power(D, alpha)) * (1 + gamma * np.power(dim / height, eta))
...
for model_size in model_size_fit_20N_list:
    for model_variants in model_variants_fit_20N_list[model_size]:
        ...
        height_fit_list.append(height_dict[model_variants])
        dim_fit_list.append(dim_dict[model_variants])
        loss_data_fit_list.append(loss_fit_20N_dict[model_variants])
        model_variants_choice_list.append(model_variants)
...
height_data_fit = np.array(height_fit_list)
dim_data_fit = np.array(dim_fit_list)
loss_data_fit = np.array(loss_data_fit_list)
print(N_data_fit)
print(D_data_fit)
print(height_data_fit)
print(dim_data_fit)
print(loss_data_fit)
popt, pcov = curve_fit(
    func,
    (N_data_fit, D_data_fit, height_data_fit, dim_data_fit),
    loss_data_fit,
    p0=[50000, 500000, 2, 0.5, 0.0011, 0.5],
    bounds=(
...
for idx in range(len(N_data_fit)):
    model_variants = model_variants_choice_list[idx]
    loss_predict = (E + A / np.power(N_data_fit[idx], alpha) + B / np.power(D_data_fit[idx], beta)) * (1 + gamma * np.power(dim_data_fit[idx] / height_data_fit[idx], eta))
...
dim_v14_1b = 3200
dim_v15_1b = 4096
loss_v1_1b = (E + A / np.power(N_v1_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v1_1b / height_v1_1b, eta))
print(f"loss_v1_1b: {loss_v1_1b}, error: {abs(loss_v1_1b - 2.8960) / 2.8960}")
# loss_v2_1b = (E + A / np.power(N_v2_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v2_1b / height_v2_1b, eta))
# print(f"loss_v2_1b: {loss_v2_1b}")
loss_v3_1b = (E + A / np.power(N_v3_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v3_1b / height_v3_1b, eta))
print(f"loss_v3_1b: {loss_v3_1b}, error: {abs(loss_v3_1b - 2.9090) / 2.9090}")
loss_v6_1b = (E + A / np.power(N_v6_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v6_1b / height_v6_1b, eta))
print(f"loss_v6_1b: {loss_v6_1b}, error: {abs(loss_v6_1b - 2.9326) / 2.9326}")
loss_v8_1b = (E + A / np.power(N_v8_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v8_1b / height_v8_1b, eta))
print(f"loss_v8_1b: {loss_v8_1b}, error: {abs(loss_v8_1b - 2.9198) / 2.9198}")
loss_v9_1b = (E + A / np.power(N_v9_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v9_1b / height_v9_1b, eta))
print(f"loss_v9_1b: {loss_v9_1b}")
loss_v10_1b = (E + A / np.power(N_v10_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v10_1b / height_v10_1b, eta))
print(f"loss_v10_1b: {loss_v10_1b}")
loss_v11_1b = (E + A / np.power(N_v11_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v11_1b / height_v11_1b, eta))
print(f"loss_v11_1b: {loss_v11_1b}")
loss_v12_1b = (E + A / np.power(N_v12_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v12_1b / height_v12_1b, eta))
print(f"loss_v12_1b: {loss_v12_1b}")
loss_v13_1b = (E + A / np.power(N_v13_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v13_1b / height_v13_1b, eta))
print(f"loss_v13_1b: {loss_v13_1b}")
loss_v14_1b = (E + A / np.power(N_v14_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v14_1b / height_v14_1b, eta))
print(f"loss_v14_1b: {loss_v14_1b}")
loss_v15_1b = (E + A / np.power(N_v15_1b, alpha) + B / np.power(D_1b, beta)) * (1 + gamma * np.power(dim_v15_1b / height_v15_1b, eta))
print(f"loss_v15_1b: {loss_v15_1b}")
...
for model_size in model_size_test_20N_list:
    for model_variants in model_variants_test_20N_list[model_size]:
        ...
        dim_data_test_list.append(dim_dict[model_variants])
        height_data_test_list.append(height_dict[model_variants])
...
dim_data_test = np.array(dim_data_test_list)
height_data_test = np.array(height_data_test_list)
...
for idx in range(len(N_data_test)):
    ...
    loss_predict = (E + A / np.power(N_data_test[idx], alpha) + B / np.power(D_data_test[idx], beta)) * (1 + gamma * np.power(dim_data_test[idx] / height_data_test[idx], eta))
